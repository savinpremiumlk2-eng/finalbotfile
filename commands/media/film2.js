...
module.exports = { 
    ...
    async execute(sock, msg, args) {
        ...
        const waitMsg = await msg.reply('📥 *Infinity MD is fetching your movie... Please wait.*');
        
        try {
            const results = await cinedl(args[0]);
            if (!results || results.length === 0) return msg.reply('No download links found.');
            ...
            if (!downloadUrl) {
                return msg.reply('Could not find a direct download link for automatic upload. Please use the links from .movie command.');
            }
